familys = {'Артем': {'отец - Генадий': 'дед - Абдул'},
           'Петр': {'отец - Эдуард': 'дед Аркадий'},
           'Николай': {'отец - Игнат': 'дед - Сергей'}}
menu = ("""
            Родственники
        0 - Выход.
...
while choice != 0:
    ...
    if choice == 0:
        ...
        name = input('\nВведите имя?: ')
        if name in familys:
            print(f"\nРодителем человека по имени {name} является\
                {familys[name[0]]} а его дедом {familys[name[1]]}")
        else:
